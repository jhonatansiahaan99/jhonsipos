...
require "../module/mode-data-jual.php";

$title = "Edit Transaksi - Jhonsi Bengkel Motor";
require "../template/header.php";
require "../template/navbar.php";
require "../template/sidebar.php";

//update
$idEdit = isset($_GET['id']) ? $_GET['id'] : '';
$tglEdit = isset($_GET['tgl']) ? $_GET['tgl'] : '';
// $idEdit = $_GET['id'];
// $tglEdit = $_GET['tgl'];

$sqlJualHead = $koneksi->query("SELECT * FROM tbl_jual_head WHERE NO_JUAL = '$idEdit' AND TGL_JUAL = '$tglEdit' ");
$jualHead = $sqlJualHead->fetch_assoc();
    ...
    echo "<script>
            setTimeout(function() {
                swal({
                    ...
                    window.location.href = '?id=$idjual&tgl=$tgl';
                });
            }, 100);
        </script>";
}
        ...
        echo "<script>
            setTimeout(function() {
                swal({
                    ...
                    window.location.href = 'edit-data-penjualan.php?id=$idEdit&tgl=$tgl';
                });
            }, 100);
        </script>";
    }
...
if (isset($_POST['addbrg'])) {
    ...
    if (insert($_POST)) {
        echo "<script>
                document.location  = '?id=$idEdit&tgl=$tgl';
        </script>";
    }
}
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item"><a href="<?= $main_url ?>data-penjualan">Data Penjualan</a></li>
                    ...
                    <div class="col-lg-6">
                        <div class="card card-outline card-warning p-3">
                            <div class="form-group row mb-2">
                                ...
                                <div class="col-sm-4">
                                    <input type="text" name="nojual" class="form-control" id="noNota" value="<?= $idEdit ?>" readonly>
                                </div>
                                <label for="tglNota" class="col-sm-2 col-form-label">Tgl Nota</label>
                                <div class="col-sm-4">
                                    <input type="date" name="tglNota" class="form-control" id="tglNota" value="<?= $tglEdit  ?>" required onchange="updateTglNotaCustomer()" readonly>
                    ...
                    <div class="col-lg-6">
                        <div class="card card-outline card-danger pt-3 px-3 pb-2">
                            ...
                            <h1 class="font-weight-bold text-right" style="font-size:70pt;">
                                <input type="hidden" name="total" id="total" value="<?= totalJual($idEdit) ?>"><?= number_format(totalJual($idEdit), 0, ',', '.') ?>
                            </h1>
                        </div>
                    </div>
                </div>
                ...
                <div class="card card-outline card-success table-responsive px-2">
                    <table class="table table-sm table-hover text-nowrap">
                        ...
                        <tbody>
                            <?php
                            $no = 1;
                            $brgDetail  = getData("SELECT * FROM tbl_jual_detail WHERE NO_JUAL = '$idEdit' AND TGL_JUAL ='$tglEdit' ");
                ...
                <div class="row">
                    <div class="col-lg-4 p-2">
                            ...
                            <div class="col-sm-9">
                                <select name="customer" id="customer" class="form-control form-control-sm select2">
                                    ...
                                    foreach ($customers as $customer) {
                                        // Tandai opsi customer terpilih
                                        $selected = ($customer['NAMA'] === $jualHead['CUSTOMER']) ? 'selected' : '';
                                    ?>
                                        <option value="<?= $customer['NAMA'] ?>" <?= $selected ?>><?= $customer['NAMA'] ?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
                        ...
                        <div class="form-group row bm-2">
                            ...
                            <div class="col-sm-9">
                                <textarea name="keterangan" id="keterangan" class="form-control form-control-sm"><?= $jualHead['KETERANGAN'] ?></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4 py-2 px-3">
                        <div class="form-group row mb-2">
                            ...
                            <div class="col-sm-9">
                                <input type="text" name="bayar" class="form-control form-control-sm text-right FormatUang bayarpenjualan" value="<?= $jualHead['JML_BAYAR'] ?>" autocomplete="off" id="bayar">
                            </div>
                        </div>
                        <div class="form-group row mb-2">
